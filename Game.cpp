...
void Game::RunDifficultyWindow() {
    ...
    sf::RenderWindow difficultyWindow(sf::VideoMode(500, 500), "Difficulty Selection");
        ...
        while (difficultyWindow.pollEvent(ev)) {
            ...
            if (ev.type == sf::Event::MouseButtonPressed) {
                ...
                if (easyButton.getGlobalBounds().contains(ev.mouseButton.x, ev.mouseButton.y)) {
                    ...
                    sf::RenderWindow customWindow(sf::VideoMode(400, 300), "Custom Settings");
                    sf::Text widthText("Width (max 30):", Font, 18); // Добавлено ограничение
                    widthText.setPosition(50, 50);
                    sf::Text heightText("Height (max 16):", Font, 18); // Добавлено ограничение
                            ...
                            if (customEvent.type == sf::Event::TextEntered) {
                                if (customEvent.text.unicode == '\b') {
                                    ...
                                    if (enteringWidth) {
                                        ...
                                        const int maxMines = inputWidth * inputHeight - 1; // Максимум мин - все клетки кроме одной
                ...
                // Обработка опций
                    ...
                    if (allowMistakes) {
                        sf::RenderWindow mistakesWindow(sf::VideoMode(300, 150), "Set Mistakes Allowed");
...
void Game::openSettingsWindow() {
    modalWindowOpen = true;
    sf::RenderWindow settingsWindow(sf::VideoMode(400, 300), "Mouse Button Settings");
        ...
        while (settingsWindow.pollEvent(ev)) {
            ...
            if (ev.type == sf::Event::MouseButtonPressed) {
                if (settingOpenButton) {
                    ...
                    flagCellButton = ev.mouseButton.button;
                    settingsWindow.close();
                }
            }
        }
...
void Game::displayHighScores() {
    // Оставляем как есть
    modalWindowOpen = true;
    sf::RenderWindow hsWindow(sf::VideoMode(400, 500), "High Scores");
    ...
    if (flaggedMines == map[y][x])
    {
        for (auto& direction : directions)
        {
            ...
            if (newX >= 0 && newX < mapSizeX && newY >= 0 && newY < mapSizeY)
            {
                ...
                {
                    if (map[newY][newX] == static_cast<int>(CellState::Mine))
                    {
                        gameOver();
                        return;
                    }
            ...
            else if (!gameLost && !gameWon)
            {
                if (x < mapSizeX && x >= 0 && y < mapSizeY && y >= 0)
                {
                    ...
                    if (e.mouseButton.button == openCellButton)
                    {
                        ...
                        if (map[y][x] == static_cast<int>(CellState::Mine)) // Если кликнули на мину
                        {
                            hoveredX = x;
                            hoveredY = y;
